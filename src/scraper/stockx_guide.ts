...
class StockX {
  ...
  collectAll = async (): Promise<Array<string>> => {
    ...
    await brands.map((e: { name: string; models: Array<string> }) => {
      e.models.map((v: string) => {
        if (e.name == v) _tags.push(e.name);
        else _tags.push(e.name + "," + v);
      });
    });
  ...
  checkProductURL = async (baseUrl: string): Promise<any> => {
    ...
    if (
      newURL.length > 0 &&
      newURL[0].ProductAmount == response.data.Pagination.total
    ) {
      const checking: any = await prisma.products({
        where: { urlForCheck: baseUrl }
      });
      if (newURL[0].ProductAmount > checking.length) {
        await console.log("[Warning] There are unregistered data in ", baseUrl);
        await prisma.updateURL({
    ...
    if (newURL.length == 0) {
      ...
      await console.log(
        `Create URL information: ${total} items, ${lastPage} pages`
      );
    } else {
  ...
  collectProductsInPage = async (
    ...
    if (!response || !response.data) {
    ...
    } else if (response.data.Pagination.total == 0) {
      // case 2
      return;
    } else {
      //case 3
      ...
      if (response.data.Pagination.nextPage)
        ...
        // It start from "/api/...""
        setTimeout(() => {
          this.collectProductsInPage(baseUrl, page + 1);
        }, 1000);
    }
  };
